...
local function applyOngoingDamageAdjustment(nodeActor, nodeEffect, rEffectComp)
	-- EXIT IF EMPTY REGEN
	if #rEffectComp.dice == 0 and rEffectComp.mod == 0 then return end
	...
	if rEffectComp.type == 'REGEN' then
		...
		if nPercentWounded <= 0 then return end
		-- Regeneration does not work once creature falls below 1 hit point
		if nPercentWounded >= 1 then return end
	...
	elseif rEffectComp.type == 'TEMPO' or rEffectComp.type == 'TEMPA' then
		...
		if #rEffectComp.remainder > 0 then table.insert(aResults, '[TYPE: ' .. string.lower(table.concat(rEffectComp.remainder, ',')) .. ']') end
	end
	...
	if EffectManager.isGMEffect(nodeActor, nodeEffect) then rRoll.bSecret = true end
...
function onEffectActorStartTurn(nodeActor, nodeEffect)
	...
	for _, sEffectComp in ipairs(aEffectComps) do
		...
		elseif rEffectComp.type == 'IF' then
			local rActor = ActorManager.resolveActor(nodeActor)
			if not EffectManager4E.checkConditional(rActor, nodeEffect, rEffectComp) then break end
			...
			if nActive == 2 then
				...
				elseif rEffectComp.mod > 0 then
					...
					elseif rEffectComp.type == 'TEMPO' then
						...
						if not aFixedEffectDMGO[nodeActor] then aFixedEffectDMGO[nodeActor] = {} end
